...
Front Panel Display can be helpful when you have renamed an input source in your AVR:
  ...
  note 2: when using Spotify or other streaming services the Front Panel Display on the AVR might be scrolling though some info like song and artist, the sensor of this integration will only be updated with the value of the selected 'sub-source' of 'NET' which are TuneIn, Spotify, Deezer, Tidal, AmazonMusic, Chromecast, DTS-Play-Fi, AirPlay, Alexa, Music-Server, USB, Play-Queue. This prevents the integration to continuously update the sensor.
