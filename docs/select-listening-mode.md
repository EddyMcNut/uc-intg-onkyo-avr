...
As from v0.8.0 this integration offers a 'select' entity for Listening Mode.
...
### Customizing the `listening_mode` select
...
- Behavior:
  - If you provide a list, the `listening_mode` select-entity will show *only* those options for that AVR regardless of the current source/activity.
